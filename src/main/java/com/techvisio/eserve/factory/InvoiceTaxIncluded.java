...
public class InvoiceTaxIncluded implements InvoiceGenerator{

	CacheManager cacheManager;

	public InvoiceTaxIncluded(CacheManager cacheManager) {
	...
	public Invoice generateInvoice(Double amount){
		...
		basePrice = basePrice*amount/totalAmount;
		basePrice = Math.round(basePrice*100.0)/100.0;
		totalAmount = basePrice;
		amountMap.put(AppConstants.BASE_AMOUNT_ID, basePrice);
		amountMap.put(AppConstants.TOTAL_AMOUNT_ID, totalAmount);

		totalAmount = calculateComponentAmount(basePrice, totalAmount,
				invoiceTaxes, amountMap);
